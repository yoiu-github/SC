...
/// * `tier` - tier number
pub fn set_tier<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    env: Env,
    config: &Config,
    priority: u8,
    ...
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::SetTier { status: Success })?),
    })
}
...
pub fn query<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>, msg: QueryMsg) -> QueryResult {
    let response = match msg {
        ...
        QueryMsg::TierOf { token_id } => query_tier_of(&deps.storage, &token_id),
...
/// * `storage` - a reference to the contract's storage
/// * `token_id` - string slice of the token id
pub fn query_tier_of<S: ReadonlyStorage>(storage: &S, token_id: &str) -> QueryResult {
    ...
    if let Some(idx) = may_idx {
        ...
        to_binary(&QueryAnswer::TierOf { tier: token.tier })
    } else {
        Err(StdError::generic_err(format!(
            "Token ID: {} not found",
            token_id
