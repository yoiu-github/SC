...
#[cfg(test)]
pub use manual::get_tier_index;

#[cfg(not(test))]
pub use query::get_tier_index;

#[cfg(test)]
mod tests {
    use super::manual::{get_tier_index, set_tier};
    ...
    fn manual_tier() {
        ...
        for i in 1..100 {
            set_tier(i);
            assert_eq!(get_tier_index(&deps, address.clone(), None), Ok(i - 1));
        }
    }
}
